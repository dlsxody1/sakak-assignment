...
**대시보드를 먼저 끝내고 `algorithm/`으로 간다.** 2단계 인증과 배포는 끝났다 —
남은 큰 구멍은 `algorithm/`에 소스가 없다는 것이다.

**CANDiY는 CORS 헤더를 주지 않는다** (확인함: `OPTIONS` → 403). 브라우저 직접 호출이
불가능해 [`dashboard/api/checkup.ts`](dashboard/api/checkup.ts) 서버리스 프록시가 유일한
경로이고, 그 덕에 `API_KEY`가 번들에 박히지 않는다 — 두 문제가 한 결정으로 풀렸다.
`VITE_` 접두사를 붙이는 순간 그게 깨진다.
