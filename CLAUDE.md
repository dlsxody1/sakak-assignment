...
**UI·화면을 설계할 때는 `impeccable` 스킬을 반드시 사용한다.** 선택이 아니다.
대시보드 레이아웃, 판정 색상 체계, 차트 구성, 반응형 동작이 모두 대상이다.

평가 기준에 "UI 설계 — 대시보드 구조, 시각화의 적절성, 반응형, 로딩/에러 상태 처리"가 있다.

## 아직 정하지 않은 것

확정된 척하지 않는다. 구현하면서 정한다:

- 차트 라이브러리, 판정 색상 체계 → **`impeccable`을 거쳐 정한다** (자유 선택 아님)
- 로그인 Mock 방식, 라우트 경로 구조
- 판정 5-state의 최종 명명
- 성별 입력 경로 (API가 안 주는 값)
- 배포 방식 (Vercel / Dockerfile)
